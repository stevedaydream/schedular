...
主班表頂部的橫向步驟卡，依偵測狀態顯示 7 步進度並自動展開當前步驟的動作按鈕：
①本月人員 → ②六日輪序（weekendFilled）→ ③配額帶入（試算/帶入/結算）→ ④處理預約（待確認數）→ ⑤自動填入（完成度%）→ ⑥檢查異常（健康度）→ ⑦鎖定班表。
健康度資訊（完成度／異常）已合併於 ⑤⑥；主要動作按鈕移入對應步驟展開區。
頂部工具列僅保留：月份切換、顯示預約、CSV、列印，與「⋯ 更多」下拉（移交排班者、清除班表）。
